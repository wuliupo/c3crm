...
class MemdaysDataPopulator extends CRMEntity {
	...
	function create_defautdata($is_upgrade=false) {
		...
		if($noofrows > 0) {
			...
			$this->db->query("insert into ec_tab values (".$tab_id.",'Memdays',0,".$tab_id.",'Memdays',null,null,1,1)");
		...
		$customview_sql = "INSERT INTO ec_customview(cvid,viewname,setdefault,setmetrics,entitytype,setpublic) VALUES('".$cvid."','所有','1','0','Memdays','0')";
		$this->db->query($customview_sql);
		$customview_sql = "INSERT INTO ec_cvcolumnlist VALUES (".$cvid.", 0, 'ec_memdays:memdayname:memdayname:Memdays_Memday_Name:V')";
		$this->db->query($customview_sql);
		$customview_sql = "INSERT INTO ec_cvcolumnlist VALUES (".$cvid.", 1, 'ec_memdays:smownerid:assigned_user_id:Memdays_Assigned_To:V')";
		$this->db->query($customview_sql);
