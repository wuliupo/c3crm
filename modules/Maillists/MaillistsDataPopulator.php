 ...
 */
class MaillistsDataPopulator extends CRMEntity {
		
	function MaillistsDataPopulator() {
		$this->log = LoggerManager::getLogger('MaillistsDataPopulator');
	...
	function create_defautdata($is_upgrade=false) {
		...
		$query = "select tabid from ec_tab where name='Maillists'";
		...
		if($noofrows > 0) {
			...
			$this->db->query("insert into ec_tab values (".$tab_id.",'Maillists',0,".$tab_id.",'Maillists',null,null,1,1)");
