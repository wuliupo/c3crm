 ...
 */
class QunfatmpsDataPopulator extends CRMEntity {
	...
	function create_tables() {
		$success = $this->db->createTables("modules/Qunfatmps/Schema.xml");
		if($success == 0) {
			echo ("Error: 创建数据库表失败.\n");
		}
		elseif ($success == 1) {
	...
	function create_defautdata($is_upgrade=false) {
		...
		$query = "select tabid from ec_tab where name='Qunfatmps'";
		...
		if($noofrows > 0) {
			...
			$this->db->query("insert into ec_tab values (".$tab_id.",'Qunfatmps',0,".$tab_id.",'Qunfatmps',null,null,1,1)");
		...
		include('modules/Qunfatmps/ModuleConfig.php');
		
		$this->insert_def_org_modulefield($tab_id);
		$this->db->query("insert into ec_entityname values(".$tab_id.",'Qunfatmps','ec_qunfatmps','qunfatmpname','qunfatmpsid')");
		//custom view
		...
		$customview_sql = "INSERT INTO ec_customview(cvid,viewname,setdefault,setmetrics,entitytype,setpublic) VALUES('".$cvid."','所有','1','0','Qunfatmps','0')";
		$this->db->query($customview_sql);
		$customview_sql = "INSERT INTO ec_cvcolumnlist VALUES (".$cvid.", 0, 'ec_qunfatmps:qunfatmpname:qunfatmpname:Qunfatmps_Qunfatmp_Name:V')";
		$this->db->query($customview_sql);
		$customview_sql = "INSERT INTO ec_cvcolumnlist VALUES (".$cvid.", 1, 'ec_qunfatmps:smownerid:assigned_user_id:Qunfatmps_Assigned_To:V')";
		$this->db->query($customview_sql);
