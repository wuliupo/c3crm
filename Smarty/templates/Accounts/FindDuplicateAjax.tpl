		...
		{/if}
			{foreach key=key3 item=newdata1 from=$data}
				<tr class="{$tdclass}" nowrap="nowrap" >
					...
					{if $cnt2 eq 0}
						<td align="center" rowspan='{$cnt}'><input class="crmbutton small edit" name="merge" value="{$MOD.LBL_MERGE}" onclick="merge_fields('{$key1}','{$MODULE}','{$CATEGORY}');" type="button"></td>
					{/if}
